...
const LanguageSwitcher = () => {
  ...
  const languages: { code: Language; name: string; flag: React.ReactNode }[] = [
    { code: 'en', name: 'English', flag: <span className="inline-block w-5 text-left">🇬🇧</span> },
    { code: 'es', name: 'Español', flag: <span className="inline-block w-5 text-left">🇪🇸</span> },
    {
      code: 'ca',
      name: 'Català',
      flag: (
    ...
    { code: 'pt', name: 'Português', flag: <span className="inline-block w-5 text-left">🇧🇷</span> },
  ]
    ...
    <div className="relative group">
      ...
      <div className="absolute top-full right-0 mt-2 bg-background rounded-2xl shadow-pop border border-border backdrop-blur-sm opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 z-50 min-w-[150px] overflow-hidden">
        {languages.map((lang) => (
          ...
          >
            <span className="text-base flex items-center">{lang.flag}</span>
            <span>{lang.name}</span>
          </button>
        ))}
      </div>
